...
class Settings(BaseSettings):
    ...
    approval_mode: ApprovalMode = Field(
        default=ApprovalMode.STRICT,
        description="Approval strictness level"
    )
    
    block_destructive_actions: bool = Field(
        default=True,
        description="Block potentially destructive actions regardless of approval"
    )
    
    require_justification_for_exploitation: bool = Field(
        default=True,
        description="Require written justification before exploitation attempts"
    )
    
    audit_all_approvals: bool = Field(
        default=True,
        description="Log all approval decisions for audit trail"
    )
