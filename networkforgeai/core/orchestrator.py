...
class ScanOrchestrator:
    ...
    async def execute_scan(self):
        ...
        try:
            ...
            vuln_agents = [a for a in self.agents.values()
                         if "vulnerability_scanning" in a.get_capabilities()]
            if vuln_agents:
            ...
            exploit_agents = [a for a in self.agents.values()
                            if "exploitation" in a.get_capabilities()]
            if exploit_agents:
            ...
            post_exploit_agents = [a for a in self.agents.values()
                                  if "post_exploitation" in a.get_capabilities()]
    ...
    async def from_state(cls, scan_id: str, save_base_dir: str = "./scans") -> "ScanOrchestrator":
        ...
        orchestrator.started_at = datetime.fromisoformat(state["started_at"]) if state["started_at"] else None
