...
class HydraTool(BaseTool):
    ...
    def parse_findings(self, stdout: str, stderr: str) -> List[Dict[str, Any]]:
        ...
        success_pattern = r'\[(\w+)\]\/(\w+).*host:\s*([^\s]+)\s+login:\s*([^\s]+)\s+password:\s*([^\s]+)'
        ...
        for match in simple_matches:
            ...
            # Avoid duplicates
            if not any(f.get("username") == username and f.get("password") == password 
                      for f in findings):
                findings.append(finding)
...
class ImpacketTools(BaseTool):
    ...
    def build_command(self, target: str, options: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        if tool not in self.available_tools:
            raise ValueError(f"Unknown impacket tool: {tool}. Available: {list(self.available_tools.keys())}")
